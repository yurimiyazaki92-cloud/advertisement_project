...
void HD44780::Initialize(void) {
  ...
  uint8_t Aumlaut[] = swed_a_umlaut;
  CreateChar(4, Aumlaut);
  uint8_t Oumlaut[] = swed_o_umlaut;
  CreateChar(5, Oumlaut);
  uint8_t Aring[] = swed_a_ring;
  CreateChar(6, Aring);
...
void HD44780::WriteSwedish(const char *text)
{
    int charCount = 0;  
    ...
    while (*text) {
        ...
        if ((uint8_t)text[0] == 0xC3) {
            uint8_t second = (uint8_t)text[1];

            switch (second) {
                case 0xA4: WriteData(4); break; // ä
                case 0xB6: WriteData(5); break; // ö
                case 0xA5: WriteData(6); break; // å
                default:   WriteData('?'); break;
            }
